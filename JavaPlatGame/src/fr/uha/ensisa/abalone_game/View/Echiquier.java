...
//Boules noires
   ...
   point_n[13]= new Point(200, 122);
	...
	public void placer()
	{
		for(int i=0; i<14; i++)
	    {
	    	boule_n[i] = new BouleJoueur(point_n[i],"/fr/uha/ensisa/abalone_game/Resources/boule_n.png");
		 ...
		 for(int i=0; i<14; i++)
		    {
		    	...
		    	boule_b[i] = new BouleJoueur(point_b[i],"/fr/uha/ensisa/abalone_game/Resources/boule_b.png");
...
public void paintBorder(Graphics forme){ 
	...
	Image img=null;
	try {
		img = ImageIO.read(getClass().getResource("/fr/uha/ensisa/abalone_game/Resources/Trou2.png"));
	} catch (IOException e) {
...
public int getIDB(Object a)			//récupéré l'ID de la boule a partir de son instance
{
...
public void setBState(int id,boolean etat,Point position)		//affiche la boule a sa position courante seulement pour ls joueur humain,si elle sortie on l'a fait disparaitre
{
	if(id<14)
	{
		for(BouleJoueur v:boule_n)
